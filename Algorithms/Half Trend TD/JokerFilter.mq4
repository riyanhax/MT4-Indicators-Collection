...
#property copyright "Copyright 2013 © Jack London"
#property link      "www.joker.com"
...
int init() {
   ...
   IndicatorShortName(f0_1(g_timeframe_124) + "JokerFilter");
   return (0);
}
...
void f0_6(int ai_0, string as_4) {
   ...
   if (gs_nothing_144 != as_4 || g_time_152 != Time[ai_0]) {
      ...
      str_concat_12 = StringConcatenate(Symbol(), " at ", TimeToStr(TimeLocal(), TIME_SECONDS), " JokerFilter direction changed to", as_4);
      if (alertsMessage) Alert(str_concat_12);
      if (alertsEmail) SendMail(StringConcatenate(Symbol(), "JokerFilter"), str_concat_12);
