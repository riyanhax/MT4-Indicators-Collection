...
input int      LabelShift = 250;
input bool     ShowVolumeLabels = true;
input color    WaveColor  = clrRed;
input int      WaveWidth  = 2;
   ...
   for(int i=limit-1; i>=0; i--) {
      ...
      if (waveDirection[i] != waveDirection[i+1]) {
         ...
         for (int k=waveChangeBar-1; k>=i; k--) {
            ...
            if (waveDirection[i] ==  1) {
               upVolumeBuffer[k] = double(volumeTracker);
               dnVolumeBuffer[k] = 0;
            }
            if (waveDirection[i] == -1) {
               upVolumeBuffer[k] = 0;
               dnVolumeBuffer[k] = double(volumeTracker);
            }
         }
         ...
         if (ShowVolumeLabels == true) {
            ...
            if (waveDirection[i] == 1) {
               ...
               ObjectSetText(volLabel, DoubleToString(dnVolumeBuffer[waveChangeBar], 0), 8, NULL, clrFireBrick);
            }
            else{
               ObjectCreate(0, volLabel, OBJ_TEXT, 0, time[waveChangeBar], high[waveChangeBar]+shift);
               ObjectSet(volLabel, OBJPROP_ANGLE, 90);
               ObjectSet(volLabel, OBJPROP_ANCHOR, ANCHOR_LEFT);
               ObjectSetText(volLabel, DoubleToString(upVolumeBuffer[waveChangeBar], 0), 8, NULL, clrDarkGreen);
            }
         }
      }
      else {
      ...
      // Set the indicators
      if (waveDirection[i] ==  1) {
         upVolumeBuffer[i] = double(volumeTracker);
         dnVolumeBuffer[i] = 0;
      }
      if (waveDirection[i] == -1) {
         upVolumeBuffer[i] = 0;
         dnVolumeBuffer[i] = double(volumeTracker);
      }
   }
