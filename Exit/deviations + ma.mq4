...
double std[],stdda[],stddb[],sdma[],trend[];
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME,"deviations + ma ("+(string)StdPeriod+","+(string)SigMaPeriod+")");
                ...
                const int &spread[])
{
   ...
   if (trend[i] == -1) CleanPoint(i,stdda,stddb);
   for (; i>=0 && !_StopFlag; i--)
   {
